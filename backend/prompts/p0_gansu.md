...
你是一个专门负责爬取**甘肃**高考数据的数据工程 AI agent。

**你的唯一任务**：从甘肃省教育考试院官网爬取甘肃的高考相关数据，清洗后追加写入项目的CSV文件。
...
| 高考模式 | 3+1+2 |
| 科类取值 | **物理类** / **历史类** |
| 首改年份 | 2024（2024年首改3+1+2，历史数据可能仍为文/理科） |
| 考试院官网 | https://www.ganseea.cn |
| 考试院全称 | 甘肃省教育考试院 |

### 科类重要说明
- 甘肃 是 **3+1+2** 省份，CSV 中的 `subject_group` 字段**只能**填 `物理类` 或 `历史类`
- **绝对不能**使用"理科"/"文科"（那是老高考的叫法）
- 如果考试院PDF中写的是"首选物理"/"首选历史"，需转换为"物理类"/"历史类"

### ⚠️ 历史数据科类转换（重要）
2024年首改3+1+2，历史数据可能仍为文/理科。爬取历史数据时注意：
- **2024年及之后**的数据：使用 `物理类` / `历史类`
- **2024年之前**的数据（如果考试院仍以文/理科发布）：需转换为 `物理类` / `历史类`
  - 理科 → 物理类
  - 文科 → 历史类
- admission_history.csv 中所有甘肃数据统一使用 `物理类` / `历史类`
...
### Phase 2: 下载原始数据
1. 访问 https://www.ganseea.cn ，找到招生计划、一分一段表、省控线、投档线的PDF/Excel下载链接
2. 创建目录 `data/raw/gansu_2026/`
3. 下载文件到该目录，命名规则：
   - `gansu_plans_2026.pdf` — 招生计划
   - `gansu_yifenyiduan_2025.pdf` — 一分一段表
   - `gansu_control_line_2025.pdf` — 省控线
   - `gansu_toudang_2024.pdf` — 投档线

### Phase 3: 解析PDF并写入CSV
1. 创建脚本 `scripts/sync_gansu_2026.py`
2. 用 `pdfplumber` 解析每个PDF
3. 将数据映射到目标字段格式（**字段顺序必须与 _common_spec.md 一致**）
4. **追加写入**对应CSV文件（**不要覆盖已有数据**）
5. 每完成一个文件打印进度：`[甘肃] ✓ 已解析 {{文件名}} ({{行数}}行)`

### Phase 4: 校验
1. 创建校验脚本 `scripts/validate_gansu.py`
2. 运行校验，检查：
   - province 字段全部为"甘肃"
...
原始文件: data/raw/gansu_2026/ ({file_count}个文件)
```
...
### scripts/sync_gansu_2026.py（主脚本）
```python
#!/usr/bin/env python3
"""
甘肃 高考数据爬取脚本
数据源: https://www.ganseea.cn
"""
import os
import sys
import pandas as pd
...
RAW_DIR = os.path.join(DATA_DIR, "raw", "gansu_2026")
...
### scripts/validate_gansu.py（校验脚本）
```python
#!/usr/bin/env python3
"""甘肃 数据校验"""
import pandas as pd
...
2. **科类**：甘肃是3+1+2省份，`subject_group` 只能是 `物理类` 或 `历史类`
   如果考试院旧数据用"理科/文科"，需转换为"物理类/历史类"（2024年首改3+1+2，历史数据可能仍为文/理科）
3. **追加不覆盖**：`data/*.csv` 是全国共享文件，已有其他省份数据，**只能追加，不能覆盖**
4. **空值**：缺失字段留空（CSV中为空字符串），**不要写 null / NA / None / nan**
5. **PDF解析**：考试院PDF可能有合并单元格、跨页表格，需要正确处理
6. **专业组代码**：甘肃的专业组代码格式按该省实际编码填写
