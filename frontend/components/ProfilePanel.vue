  ...
  <div class="profile-panel glass-card" :class="{ scanning, incomplete: !profileStore.isProfileComplete }">
    ...
    <Transition name="extract-hint">
      <div v-if="scanning" class="extract-hint">
        ...
        <span class="extract-text">正在提取档案特征</span>
      </div>
    </Transition>
    ...
    <div class="vision-bar" :class="{ 'vision-warning': scanning && profileStore.profile.vision_status && profileStore.profile.vision_status !== '正常' }">
      ...
      <Transition name="vision-warn">
        <div v-if="scanning && profileStore.profile.vision_status && profileStore.profile.vision_status !== '正常'" class="vision-warn-text">
          <Icon name="alert" :size="11" /> {{ profileStore.profile.vision_status }} → 重点审查项
        </div>
      </Transition>
    </div>
...
.score-value { font-size: 48px; font-weight: 200; color: #f1f5f9; line-height: 1; letter-spacing: -3px; }
.score-unit { font-size: 16px; color: #64748b; margin-left: 4px; }
...
.row-value { font-size: 13px; color: #e2e8f0; font-weight: 600; }
...
.completion-fill { height: 100%; background: linear-gradient(90deg, #fbbf24, #38bdf8); border-radius: 2px; transition: width 0.5s ease; }
.completion-text { font-size: 9px; color: #64748b; margin-top: 4px; display: block; }

/* ── 选科成绩 ── */
.elective-section { margin-bottom: 12px; padding-top: 10px; border-top: 1px solid rgba(255, 255, 255, 0.04); }
.elective-label { font-size: 10px; font-weight: 700; color: #64748b; text-transform: uppercase; letter-spacing: 1px; display: block; margin-bottom: 8px; }
...
.elective-name { font-size: 10px; color: #64748b; }
.elective-score { font-size: 14px; font-weight: 700; color: #e2e8f0; }

.vision-bar { margin-bottom: 12px; }
.vision-options { display: flex; gap: 4px; margin-top: 6px; }
.vision-chip { flex: 1; padding: 7px 0; text-align: center; border-radius: 8px; font-size: 12px; font-weight: 600; background: rgba(255, 255, 255, 0.04); color: #94a3b8; transition: all 0.2s; cursor: pointer; }
.vision-chip:hover { background: rgba(56, 189, 248, 0.08); color: #7dd3fc; }
.vision-chip.active { background: rgba(56, 189, 248, 0.15); color: #38bdf8; border: 1px solid rgba(56, 189, 248, 0.3); }
.profile-edit-hint { font-size: 10px; color: #475569; text-align: center; cursor: pointer; }
.profile-edit-hint:hover { color: #38bdf8; }
...
/* 扫描时分数/位次数字高频闪烁 */
.glitching { animation: text-glitch 0.3s 3; }
@keyframes text-glitch {
  0%   { color: #f1f5f9; text-shadow: none; transform: translateX(0); }
  20%  { color: #38bdf8; text-shadow: 2px 0 #fb7185, -2px 0 #818cf8; transform: translateX(-1px); }
  40%  { color: #818cf8; text-shadow: -2px 0 #fb7185, 2px 0 #38bdf8; transform: translateX(1px); }
  60%  { color: #fda4af; text-shadow: 1px 0 #38bdf8, -1px 0 #818cf8; transform: translateX(0); }
  80%  { color: #38bdf8; text-shadow: none; transform: translateX(-1px); }
  100% { color: #f1f5f9; text-shadow: none; transform: translateX(0); }
}

/* ── 扫描时的"正在提取档案特征"脉冲提示 ── */
.profile-panel.scanning { border-color: rgba(56, 189, 248, 0.3); box-shadow: 0 0 24px rgba(56, 189, 248, 0.1), inset 0 1px 0 rgba(255, 255, 255, 0.1); }
.extract-hint { display: flex; align-items: center; gap: 6px; padding: 5px 10px; margin-bottom: 10px; background: rgba(56, 189, 248, 0.1); border-radius: 8px; border: 1px solid rgba(56, 189, 248, 0.2); }
.extract-dot { width: 6px; height: 6px; border-radius: 50%; background: #38bdf8; box-shadow: 0 0 8px rgba(56, 189, 248, 0.8); animation: extract-pulse 0.8s ease-in-out infinite; }
@keyframes extract-pulse { 0%, 100% { opacity: 1; transform: scale(1); } 50% { opacity: 0.4; transform: scale(1.4); } }
.extract-text { font-size: 10px; color: #7dd3fc; font-weight: 600; letter-spacing: 1px; }
