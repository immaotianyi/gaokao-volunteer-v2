  ...
  <div class="advisor-layout">
    ...
    <div class="messages-scroll custom-scrollbar" ref="scrollRef">
      ...
      <div v-if="advisorStore.messages.length === 0" class="empty-state">
        <div class="empty-icon"><Icon name="bolt" :size="32" /></div>
        <span class="empty-title">AI 志愿顾问</span>
        <span class="empty-desc">基于 135 所大学章程 + 雪峰知识库 + 联网搜索，为你答疑解惑</span>
...
.empty-icon { color: #38bdf8; margin-bottom: 4px; }
.empty-title { font-size: 18px; font-weight: 700; color: #f1f5f9; }
.empty-desc { font-size: 12px; color: #64748b; max-width: 320px; line-height: 1.6; }
.quick-questions { display: flex; flex-direction: column; gap: 8px; margin-top: 16px; width: 100%; max-width: 320px; }
.quick-q { padding: 10px 14px; background: rgba(56, 189, 248, 0.06); border: 1px solid rgba(56, 189, 248, 0.15); border-radius: 10px; font-size: 13px; color: #7dd3fc; cursor: pointer; transition: all 0.2s; text-align: left; }
.quick-q:hover { background: rgba(56, 189, 248, 0.12); border-color: rgba(56, 189, 248, 0.3); }
...
.user-bubble { background: linear-gradient(135deg, #38bdf8, #818cf8); color: #fff; border-bottom-right-radius: 4px; }
.ai-bubble { background: rgba(255, 255, 255, 0.04); border: 1px solid rgba(255, 255, 255, 0.08); color: #e2e8f0; border-bottom-left-radius: 4px; }

/* ── AI 前置信息 ── */
.ai-prelude { margin-bottom: 8px; padding: 6px 10px; background: rgba(56, 189, 248, 0.05); border-radius: 7px; border: 1px solid rgba(56, 189, 248, 0.1); }
.live-prelude { background: rgba(129, 140, 248, 0.05); border-color: rgba(129, 140, 248, 0.1); }
.prelude-label { display: flex; align-items: center; gap: 4px; font-size: 10px; font-weight: 700; color: #7dd3fc; letter-spacing: 1px; margin-bottom: 3px; }
.live-prelude .prelude-label { color: #a5b4fc; }
.prelude-text { font-size: 11px; color: #64748b; line-height: 1.5; display: block; }

/* ── AI 正文 ── */
.ai-content { white-space: pre-wrap; word-break: break-word; overflow-wrap: break-word; }
.typing-cursor { color: #38bdf8; animation: cursor-blink 0.8s step-end infinite; }
@keyframes cursor-blink { 0%, 50% { opacity: 1; } 51%, 100% { opacity: 0; } }

/* ── 思考中动画 ── */
.ai-thinking { display: flex; gap: 4px; padding: 4px 0; }
.thinking-dot { width: 6px; height: 6px; border-radius: 50%; background: #38bdf8; animation: thinking-bounce 1.4s infinite; }
.thinking-dot.d2 { animation-delay: 0.2s; }
.thinking-dot.d3 { animation-delay: 0.4s; }
@keyframes thinking-bounce { 0%, 60%, 100% { transform: translateY(0); opacity: 0.4; } 30% { transform: translateY(-6px); opacity: 1; } }
...
.source-link { font-size: 10px; color: #38bdf8; text-decoration: none; }
...
.trust-t2 { background: rgba(56, 189, 248, 0.12); color: #7dd3fc; }
.trust-t3 { background: rgba(251, 191, 36, 0.12); color: #fde68a; }
.trust-t4 { background: rgba(148, 163, 184, 0.12); color: #94a3b8; }
...
.suggestion-chip { padding: 5px 12px; background: rgba(129, 140, 248, 0.1); border: 1px solid rgba(129, 140, 248, 0.2); border-radius: 16px; font-size: 11px; color: #a5b4fc; cursor: pointer; transition: all 0.2s; }
.suggestion-chip:hover { background: rgba(129, 140, 248, 0.18); }
...
.input-field { flex: 1; background: rgba(0, 0, 0, 0.3); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 10px; padding: 10px 14px; font-size: 13px; color: #e2e8f0; outline: none; box-sizing: border-box; }
.input-field:focus { border-color: rgba(56, 189, 248, 0.4); }
.input-field::placeholder { color: #475569; }
.send-btn { width: 40px; height: 40px; background: linear-gradient(135deg, #38bdf8, #818cf8); border-radius: 10px; display: flex; align-items: center; justify-content: center; color: #fff; cursor: pointer; transition: all 0.2s; flex-shrink: 0; }
