...
onMounted(() => {
  ...
  if (gap !== 0) {
    setTimeout(() => {
      scoreGap.start(Math.abs(gap), 600)
      // 滚动结束时触发震动
      setTimeout(() => {
        shaking.value = true
  ...
  // 瞄准框锁定
  setTimeout(() => { locked.value = true }, 200)
})
...
<template>
  <div class="radar-card" :class="[
    tierClass,
    { unlocked, 'tier-s-card': isTierS, shaking }
  ]">
    <!-- 狙击瞄准框四角 -->
    <span class="lock-corner lc-tl" :class="{ locked }" />
    <span class="lock-corner lc-tr" :class="{ locked }" />
    <span class="lock-corner lc-bl" :class="{ locked }" />
    <span class="lock-corner lc-br" :class="{ locked }" />
    ...
    <div v-if="isTierS" class="s-alert-badge">
      ...
      <span class="badge-text">捡漏预警</span>
    </div>
...
/* 震动 */
.radar-card.shaking { animation: card-shake 0.5s ease-in-out; }
...
.radar-card:hover { transform: translateY(-5px); border-color: rgba(56, 189, 248, 0.2); box-shadow: 0 12px 24px -10px rgba(0,0,0,0.5); }
.radar-card:active { background: rgba(255, 255, 255, 0.06); }
.radar-card.unlocked { border-color: rgba(56, 189, 248, 0.2); background: rgba(56, 189, 248, 0.03); }
...
.s-alert-badge { position: absolute; top: 8px; right: 8px; display: flex; align-items: center; gap: 3px; padding: 3px 8px; background: linear-gradient(135deg, rgba(250, 204, 21, 0.2), rgba(251, 191, 36, 0.15)); border: 1px solid rgba(250, 204, 21, 0.4); border-radius: 6px; z-index: 2; animation: s-badge-blink 1.5s ease-in-out infinite; }
...
/* S级卡片 hover 时瞄准框变金 */
.radar-card.tier-s-card .lock-corner.locked { border-color: rgba(250, 204, 21, 0.6); }

/* ── 狙击瞄准框四角 ── */
.lock-corner { position: absolute; width: 20px; height: 20px; border: 2px solid #facc15; opacity: 0; transform: scale(2); transition: all 0.5s cubic-bezier(0.34, 1.56, 0.64, 1); pointer-events: none; z-index: 3; }
.lock-corner.locked { opacity: 0.8; transform: scale(1); }
.lc-tl { top: 6px; left: 6px; border-right: none; border-bottom: none; border-radius: 4px 0 0 0; }
.lc-tr { top: 6px; right: 6px; border-left: none; border-bottom: none; border-radius: 0 4px 0 0; }
.lc-bl { bottom: 6px; left: 6px; border-right: none; border-top: none; border-radius: 0 0 0 4px; }
.lc-br { bottom: 6px; right: 6px; border-left: none; border-top: none; border-radius: 0 0 4px 0; }
...
.card-school { font-size: 15px; font-weight: 700; color: #f1f5f9; max-width: 160px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
.card-school-type { font-size: 10px; color: #64748b; background: rgba(255, 255, 255, 0.04); padding: 1px 6px; border-radius: 4px; }
...
.tier-a { background: rgba(56, 189, 248, 0.12); color: #7dd3fc; border: 1px solid rgba(56, 189, 248, 0.25); }
.tier-b { background: rgba(251, 191, 36, 0.12); color: #fde68a; border: 1px solid rgba(251, 191, 36, 0.25); }
.tier-c { background: rgba(148, 163, 184, 0.12); color: #94a3b8; border: 1px solid rgba(148, 163, 184, 0.25); }
.card-major { font-size: 12px; color: #94a3b8; display: block; margin: 6px 0; position: relative; z-index: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }

/* ── 数字老虎机 ── */
.card-estimate { display: flex; align-items: baseline; gap: 6px; margin-bottom: 6px; position: relative; z-index: 1; }
.est-label { font-size: 10px; color: #64748b; }
...
/* 滚动中：橙色 */
.est-value.rolling, .gap-value.rolling { color: #fb923c; text-shadow: 0 0 8px rgba(251, 146, 60, 0.5); }
/* 定格正分差：耀眼绿色 */
.est-value.positive-final, .gap-value.positive-final { color: #34d399; text-shadow: 0 0 10px #34d399, 0 0 20px rgba(52, 211, 153, 0.4); }
/* 定格负分差：红色 */
.est-value.negative-final, .gap-value.negative-final { color: #f87171; text-shadow: 0 0 8px rgba(248, 113, 113, 0.4); }
...
.tag-blue    { background: rgba(56, 189, 248, 0.1); color: #7dd3fc; }
...
.card-reason { font-size: 10px; color: #64748b; display: block; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
.trust-t2 { background: rgba(56, 189, 248, 0.12); color: #7dd3fc; border: 1px solid rgba(56, 189, 248, 0.2); }
.trust-t3 { background: rgba(251, 191, 36, 0.12); color: #fde68a; border: 1px solid rgba(251, 191, 36, 0.2); }
.trust-t4 { background: rgba(148, 163, 184, 0.12); color: #94a3b8; border: 1px solid rgba(148, 163, 184, 0.2); }

/* ── 联网动态信息 ── */
.live-info-section { margin-top: 6px; padding: 8px 10px; background: rgba(56, 189, 248, 0.04); border: 1px solid rgba(56, 189, 248, 0.1); border-radius: 7px; position: relative; z-index: 1; }
.live-row { display: flex; gap: 6px; font-size: 10px; line-height: 1.5; margin-bottom: 3px; }
.live-row:last-child { margin-bottom: 0; }
.live-label { color: #7dd3fc; font-weight: 600; flex-shrink: 0; min-width: 56px; }
.live-text { color: #94a3b8; word-break: break-all; }
.live-sources { display: flex; flex-wrap: wrap; gap: 4px; margin-top: 4px; padding-top: 4px; border-top: 1px solid rgba(56, 189, 248, 0.08); }
.source-label { font-size: 9px; color: #475569; }
.source-link { font-size: 9px; color: #38bdf8; text-decoration: none; }
